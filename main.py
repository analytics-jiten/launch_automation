...
_companyId=requests.get('https://reactor.adobe.io/companies?filter[name]=CONTAINS PLC', headers=_header, verify=False).json()['data'][0]['id']


def getPropertyId(config,access_token,companyId):
    response=requests.get("https://reactor.adobe.io/companies/"+str(companyId)+"/properties?filter[name]=CONTAINS sky",headers=_header, verify=False)
...
def createRule(config,access_token):
    ...
    response = requests.post("https://reactor.adobe.io/properties/"+propertyId+"/rules",data=json.dumps(to_python),headers=_header,verify=False)
